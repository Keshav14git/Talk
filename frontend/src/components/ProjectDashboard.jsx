...
const ProjectDashboard = ({ project }) => {
  ...
  const {
    ...
    markAllAsRead,
    isLoading,
  } = useNotificationStore(); // Added store
  ...
  const handleCommentChange = (e) => {
    ...
    if (lastAt !== -1 && lastAt >= val.length - 15) {
      // Simple check: @ recently typed
      const query = val.slice(lastAt + 1);
      if (!query.includes(" ")) {
        // Only if no space after @ yet
  ...
  const insertMention = (member) => {
    ...
    const newText =
      commentText.slice(0, lastAt) +
      "@" +
      member.fullName +
      " " +
      commentText.slice(lastAt + 1 + mentionQuery.length);
  ...
  const groupedTasks = () => {
    ...
    if (groupBy === "status") {
      ["todo", "in-progress", "blocked", "completed"].forEach(
        (s) => (groups[s] = []),
      );
      list.forEach((t) => {
  ...
  const handleDeleteSelectedComments = async (taskId) => {
    ...
    const success = await deleteTaskComments(
      taskId,
      ...
      <div className="flex-1 overflow-hidden flex flex-col relative bg-[#0a0a0a]">
                  ...
                  <div className="bg-[#212121] border border-[#2f2f2f] rounded-2xl p-6 flex flex-col justify-between group hover:border-[#424242] transition-colors shadow-sm">
                    <div className="flex items-center gap-2 mb-6">
                      <div className="size-6 rounded-full bg-white/5 flex items-center justify-center">
                        <CheckCircle className="size-3.5 text-gray-300" />
                      </div>
                    ...
                    <div>
                      ...
                      <div className="w-full bg-[#0d0d0d] rounded-full h-1 mb-3 overflow-hidden border border-[#2f2f2f]">
                        <div
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        {myTasks.slice(0, 2).map((t) => (
                          <div
                            key={t._id}
                            className="flex items-center gap-2 text-[11px] text-gray-400 bg-[#0d0d0d] px-2 py-1.5 rounded-md border border-[#2f2f2f]"
                          >
                            <div
                              className={`size-1.5 rounded-full ${t.priority === "urgent" ? "bg-red-500" : "bg-indigo-500"}`}
                            ></div>
                            <span className="truncate flex-1">{t.title}</span>
                          </div>
                        ))}
                        {myTasks.length === 0 && (
                          <div className="flex items-center gap-2 text-[11px] text-gray-500 font-medium bg-[#0d0d0d] px-2 py-1.5 rounded-md border border-[#2f2f2f] border-dashed">
                ...
                <div className="flex-1 flex flex-col space-y-6">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex bg-[#212121] p-1 rounded-lg border border-[#2f2f2f]">
                      {["all", "my", "overdue", "blocked", "completed"].map(
                        (view) => (
                          <button
                            key={view}
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="relative">
                        <select
                          ...
                          className="bg-[#212121] text-gray-300 text-xs font-bold border border-[#2f2f2f] rounded-lg px-2 py-1.5 focus:outline-none focus:border-indigo-500/50 appearance-none pr-8 cursor-pointer"
                        >
                  ...
                  <div className="space-y-6">
                    {Object.entries(taskGroups).map(
                      ([groupName, groupTasks]) => {
                        if (groupTasks.length === 0 && groupBy !== "none")
                          return null; // Hide empty groups (optional)

                        return (
                          <div
                            key={groupName}
                            className="bg-[#212121] border border-[#2f2f2f] rounded-2xl overflow-hidden shadow-sm"
                          >
                            <div className="p-3 bg-[#212121] border-b border-[#2f2f2f] flex items-between">
                              <h3 className="font-bold text-gray-200 text-sm flex items-center gap-2 capitalize">
                                {groupBy === "priority" &&
                                  getPriorityIcon(groupName)}
                                {groupName === "my"
                                  ? "My Tasks"
                                  : groupName.replace("-", " ")}
                            ...
                            <div className="divide-y divide-[#222]">
                              {groupTasks.length === 0 ? (
                                ...
                                groupTasks.map((task) => (
                                  <div
                                    key={task._id}
                                    className="group bg-[#212121] hover:bg-[#212121] transition-colors"
                                  >
                                    {/* Task Summary Row */}
                                    <div
                                    ...
                                    >
                                      ...
                                      <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-2 mb-0.5">
                                          <span
                                            className={`text-sm font-medium ${task.status === "completed" ? "text-gray-500 line-through" : "text-gray-200"}`}
                                          >
                                            {task.title}
                                          </span>
                                          {groupBy !== "priority" && (
                                        ...
                                        <div className="flex items-center gap-3 text-[10px] text-gray-500">
                                          <span
                                            className={`flex items-center gap-1 ${task.dueDate && new Date(task.dueDate) < new Date() ? "text-red-400 font-bold" : ""}`}
                                          >
                                      ...
                                      <div
                                        className={`flex items-center gap-3 transition-opacity ${task.comments?.length > 0 ? "opacity-100" : "opacity-0 group-hover:opacity-100"}`}
                                      >
                                    ...
                                    {expandedTask === task._id && (
                                      <div className="px-4 pb-4 pl-12 animate-in slide-in-from-top-2 duration-200">
                                        <div className="bg-[#212121] rounded-xl border border-[#2f2f2f] p-4">
                                          {task.description && (
                                            ...
                                            <div className="space-y-3 mb-4 max-h-40 overflow-y-auto custom-scrollbar">
                                              ...
                                              {task.comments?.map(
                                                (comment, i) => (
                                                  <div
                                                    key={i}
                                                    className={`flex gap-2 text-sm p-2 rounded-lg transition-colors ${selectedComments.has(comment._id) ? "bg-indigo-500/10 border border-indigo-500/20" : "hover:bg-white/5"}`}
                                                  >
                                                    {isSelectionMode && (
                                                      <div className="flex items-center pt-1">
                                                        <input
                                                          ...
                                                          className="appearance-none size-4 rounded border border-[#444] bg-[#2f2f2f] checked:bg-indigo-500 checked:border-indigo-500 cursor-pointer transition-all"
                                                        />
                                                      </div>
                                                    )}
                                                    <img
                                                      ...
                                                      className="size-6 rounded-full object-cover mt-0.5"
                                                    />
                                                    <div className="flex-1 min-w-0">
                                                      <div className="flex items-center gap-2">
                                            ...
                                            <div className="flex gap-2 relative">
                                              ...
                                              {showMentionDropdown &&
                                                filteredMembers.length > 0 && (
                                                  <div className="absolute bottom-full mb-2 left-0 w-64 bg-[#1f1f1f] border border-[#424242] rounded-lg shadow-xl overflow-hidden z-50 animate-in fade-in slide-in-from-bottom-2">
                                                    <div className="px-3 py-2 text-[10px] font-bold text-gray-500 uppercase tracking-wider bg-[#212121]">
                                                      Suggested Members
                                                    </div>
                                              ...
                                              <input
                                                ...
                                                onKeyDown={(e) => {
                                                  if (e.key === "Enter")
                                                    handleSendComment(task._id);
                                                  if (e.key === "Escape")
                                                    setShowMentionDropdown(
                                                      false,
                                                    );
                                                }}
                                                className="flex-1 bg-[#2f2f2f] border border-[#424242] rounded-lg px-3 py-1.5 text-sm text-white focus:outline-none focus:border-indigo-500/50"
                                              />
                                              <button
                                                onClick={() =>
                                                  handleSendComment(task._id)
              ...
              <div className="w-full xl:w-96 flex-shrink-0 space-y-8">
                ...
                <div className="bg-[#212121] border border-[#2f2f2f] rounded-2xl p-6 flex flex-col justify-between shadow-sm">
                  <div>
                    ...
                    <div className="flex -space-x-2 overflow-hidden mb-6">
                      {project.members?.slice(0, 5).map((m, i) => (
                        <img
                          key={i}
                          className="inline-block size-8 rounded-full ring-2 ring-[#212121] object-cover"
                          src={
                            m.userId?.profilePic ||
                            m.profilePic ||
                            "/avatar.png"
                      ...
                      {project.members?.length > 5 && (
                        <div className="flex items-center justify-center size-8 rounded-full ring-2 ring-[#212121] bg-[#2f2f2f] text-[10px] font-bold text-gray-400">
                          +{project.members.length - 5}
                        </div>
                      )}
                    </div>
                  ...
                  <button
                    ...
                    className="w-full py-2.5 bg-white text-black font-bold rounded-lg hover:bg-gray-200 transition-colors flex items-center justify-center gap-2 text-sm shadow-sm active:translate-y-0.5"
                  >
                ...
                <div className="bg-[#212121] border border-[#2f2f2f] rounded-2xl overflow-hidden shadow-sm">
                  <div className="p-4 border-b border-[#2f2f2f] flex items-center gap-2">
                  ...
                  <div className="p-4 space-y-4">
                    {recentActivity.length === 0 ? (
                      ...
                      recentActivity.map((task) => (
                        <div
                          key={task._id}
                          className="relative pl-4 border-l border-[#2f2f2f]"
                        >
                          ...
                          <div className="flex items-center justify-between mt-1">
                            ...
                            <span
                              className={`text-[10px] px-1.5 rounded uppercase font-bold ${getStatusColor(task.status).replace("border-gray-700", "border-transparent")}`}
                            >
                              {task.status}
                            </span>
                          </div>
        ...
        {subTab === "chat" && (
          <div className="flex flex-1 h-full overflow-hidden">
            ...
            <div className="w-64 bg-[#212121] border-r border-[#2f2f2f] flex flex-col pt-4">
              <div className="px-4 mb-4">
                ...
                <button
                  onClick={() => {
                    setActiveChatMode("general");
                    setActiveDmUser(null);
                  }}
                  className={`w-full text-left px-3 py-2 rounded-lg text-sm font-medium flex items-center gap-2 transition-all 
                                    ${activeChatMode === "general" ? "bg-[#2f2f2f] text-white" : "text-gray-400 hover:bg-[#151515] hover:text-gray-300"}`}
                >
              ...
              <div className="px-4 flex-1 overflow-y-auto custom-scrollbar">
                ...
                <div className="space-y-1">
                  {project.members?.map((member) => {
                    ...
                    const isMemberAdmin = project.admins?.includes(user._id);

                    return (
                      <div
                        key={user._id}
                        className={`w-full flex items-center justify-between px-3 py-2 rounded-lg text-sm font-medium transition-all group
                                                ${activeChatMode === "dm" && activeDmUser?._id === user._id ? "bg-[#2f2f2f] text-white" : "text-gray-400 hover:bg-[#151515] hover:text-gray-300"}`}
                      >
                        <button
                          onClick={() => {
                        ...
                        >
                          <div className="relative">
                            ...
                            />
                            {isMemberAdmin && (
                              <div className="absolute -bottom-1 -right-1 bg-indigo-500 rounded-full p-[2px]">
                                <Shield className="size-2 text-white" fill="currentColor" />
                              </div>
                            )}
                          </div>
                        ...
                        {hasManageAccess && (
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              useOrgStore.getState().updateMemberRole(
                                project._id, 
                                user._id, 
                                isMemberAdmin ? 'member' : 'admin'
                              );
                            }}
                            className={`p-1 rounded opacity-0 group-hover:opacity-100 transition-opacity ${isMemberAdmin ? 'text-indigo-400 hover:bg-indigo-500/20' : 'text-gray-500 hover:text-white hover:bg-white/10'}`}
            ...
            <div className="flex-1 flex flex-col h-full bg-[#0d0d0d]">
              {activeChatMode === "general" ? (
                <ChatContainer
                  isProjectChat={true}
                  overrideType="project"
                  overrideUser={project}
                />
              ) : (
                <ChatContainer
                  isProjectChat={false}
                  overrideType="user"
                  overrideUser={activeDmUser}
                />
              )}
            </div>
          </div>
